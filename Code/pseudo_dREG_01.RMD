...
plot_chr_distribution <- function(window_metadata, title="Chr distribution"){
...
plot_chr_distribution(pos_windows$metadata, "Chr distribution (Positive windows)")
...
# Chr distribution
plot_chr_distribution(pos_windows_filtered$metadata, "Chr distribution (Positive windows)")
...
plo_set_profiles(pos_windows_filtered, chr = "chr2") 
plo_set_profiles(pos_windows_filtered, chr = "chr3") 
plo_set_profiles(pos_windows_filtered, chr = "chr8") 
...
positive_set_granges <- GRanges(seqnames = positive_windows_filtered$metadata$chr,
                                ranges = IRanges(start = positive_windows_filtered$metadata$atac_start,
                                                 width = 501))
...
# - Risk: I will need to reiterate several times before I reach the desired number of profiles with at least a TSS
...
# Increase randomness?
bedtools shuffle -i positive_set_replicate.bed -g ../hg38.bed -excl hg38-blacklist.v2.bed -excl ../atac_positive_all_granges_501.bed

```

Method A: I start and end 7k ranges that overlap atleast with 1 CAGE (forced by bedtools shuffle)
Method B: I start with 258k ranges and end with 8k ranges that overlap atleast with 1 CAGE (completely random)
Method B123: Start with 650k ranges and end with 22k
-> if I want to reach larger size (> 10k) of the negative set, I need to generate different negative set ranges by shuffle
...
# Chr distribution
plot_chr_distribution(neg_windows$metadata, "Chr distribution (Negative windows)")
...
windows_score_distribution_by_pos_all_chr %>% 
  ggplot(aes(x = pos, y = score, color = strand)) + geom_line() +
  labs(title = "Negative set CAGE score distribution over ATAC-Seq peaks relative position",  
       x = "Relative position to ATAC mid peaks", y = "Sum of scores over windows") + 
  coord_cartesian(ylim = c(-1000, 1000)) +
  facet_wrap(~factor(chr,
                     levels = paste("chr", c(1:22, "X", "Y"), sep=""))) +
...
plo_set_profiles(neg_windows, chr = "chr2") 
plo_set_profiles(neg_windows, chr = "chr3") 
plo_set_profiles(neg_windows, chr = "chr8") 
...
# Extraction and exploration of negative range profiles (B)
...
# Extraction and exploration of negative range profiles (C)
...
plot_chr_distribution(neg_windows_sampled$metadata, "Chr distribution (Negative windows sampled)")
```
...
# Export
write_csv(windows_profile, "../Data/ML_input/profiles_replicate1_C.csv")
write_csv(windows_metadata, "../Data/ML_input/metadata_replicate1_C.csv")
write_csv(windows_profile_test, "../Data/ML_input/profiles_replicate1_C_test.csv")
write_csv(windows_metadata_test, "../Data/ML_input/metadata_replicate1_C_test.csv")
write_csv(windows_profile_train, "../Data/ML_input/profiles_replicate1_C_train.csv")
write_csv(windows_metadata_train, "../Data/ML_input/metadata_replicate1_C_train.csv")
```
